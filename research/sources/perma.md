...
2. **Cross-domain synthesis evaluation.** PERMA's multi-domain tasks (e.g., combining Travel + Calendar + Restaurant preferences) directly test whether memory systems can join information across domains. Somnigraph's LoCoMo evaluation uses single-conversation QA; the multi-hop failure analysis (`docs/benchmarks.md`) identifies vocabulary gap as the bottleneck but doesn't test cross-domain synthesis specifically. The graph edges in `scoring.py` (PPR traversal) could theoretically bridge domains, but this capability is untested.
