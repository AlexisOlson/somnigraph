...
**Why**: HingeMem's ablation shows boundary-guided memory is worth +12.8pp F1 over plain text memory (Table 3). Somnigraph's current unit is the individual `remember()` call — typically one fact or decision. For batch ingestion of conversation history, event-segmented chunks would be more natural units. The vocabulary gap problem in multi-hop retrieval (documented in `docs/benchmarks.md`) could be partially addressed by wider segments that co-locate related turns.
