...
**Why**: This directly attacks the vocabulary-gap ceiling documented in `docs/benchmarks.md`. Somnigraph currently patches multi-hop with synthetic vocabulary bridges (L5b); path propagation reaches the same target structurally, and it's an *inference-time* method (no extra LLM), unlike our sleep-built synthetics.
...
- **Contrast with Somnigraph's own multi-hop work** (`docs/benchmarks.md`, L5b synthetic bridges): M-flow reaches the same multi-hop target via inference-time path arithmetic rather than sleep-built synthetic vocabulary — the cleaner mechanism if the sub-memory graph exists.
