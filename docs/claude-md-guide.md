...
**Instruction decay over long context.** The guidance in this document weakens as the context window fills — a rule tens of thousands of tokens back gets quietly dropped. Two implications: the always-loaded snippet is kept short and front-loaded (this is *why* it's ~30 lines, not the full guide), and load-bearing behavior isn't left to a prose instruction alone. That's why `recall()` returns the feedback IDs in its output footer — the prompt to rate is coded into the tool surface rather than relying on you to remember this section. See [similar-systems.md § Practitioner signal](similar-systems.md#practitioner-signal).
