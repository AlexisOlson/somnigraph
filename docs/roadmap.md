...
**Result (2026-07-01) — split verdict; hook not built.** The offline floor study ran: discrimination is real (AUC 0.64–0.74, clearing the cliff's R² < 0 null; score magnitude beats rank-only by ~0.08 AUC), but no floor beats always-inject on F-beta at any β — the log's 0.78–0.88 usefulness base rate is a selection artifact of a log that only contains turns where recall happened. The proactive population (no-recall turns) is structurally unobservable offline. The open question is now narrower: is a **log-only online shadow hook** (observe no-recall turns, inject nothing) worth a session to collect the missing population? See [`../experiments/floor-study/findings-floor-study.md`](../experiments/floor-study/findings-floor-study.md) and the outcome header in [`proposals/proactive-injection.md`](proposals/proactive-injection.md).
...
21. **Expansion method ablation (LoCoMo).** ANSWERED (2026-07-02). The cited fire rates (rocchio 0%, multi_query 2%, entity_focus 4%; session 100%, keyword 95%, entity_bridge 96%) are a stale ≤200-search-limit instrument. At the current 4000 limit the Phase-1 pool holds the entire benchmark DB (≤861 memories), so **all six methods add zero net-new candidates** and the `--expand-all` lift (+4.9pp R@10) is entirely the Phase-2 rerank pass — proven by a Phase-2-forced/no-methods control that reproduces `--expand-all` and diverges from no-expansion on 827 records. Removability is **not settleable on this benchmark** (it cannot exercise candidate expansion); methods stay in code. See [`experiments/expansion-ablation/findings-expansion-ablation.md`](../experiments/expansion-ablation/findings-expansion-ablation.md) and `benchmarks.md` § Expansion-method ablation.
...
The forward benchmarking work: the [proposed experiments](benchmarks.md#proposed-benchmarking-experiments) (contradiction-detection rate, latency profiling), the PERMA ingest pipeline, and the remaining LoCoMo ablations (sleep, feedback — expansion methods #21 answered 2026-07-02) that would let `experiments.md` document the retrieval arc end to end.
