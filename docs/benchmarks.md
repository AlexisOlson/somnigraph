...
Where Somnigraph's numbers sit next to other systems. This is the evidence cluster's quantitative half: retrieval quality and end-to-end QA on public benchmarks, the failure analysis behind the hardest category, and the cross-system score tables with the caveats that make them (in)comparable.

For *how* the system was tuned — the studies, the methodology, the negative results — see [`experiments.md`](experiments.md). For a feature-level comparison against other memory systems, see [`similar-systems.md`](similar-systems.md). This document is the scores themselves.

Headline: **85.1% LoCoMo overall QA accuracy** (Opus judge), beating Mem0 (66.88 J), Mem0g (68.44 J), and the full-context baseline (72.90 J). Retrieval reaches **95.4% R@10** at Level 5b; the remaining gap is reader extraction, not retrieval.

## Contents

1. [LoCoMo retrieval benchmark](#locomo-retrieval-benchmark) — the setup and comparability caveat
2. [Run history](#run-history) — Level 0 → 5b
3. [End-to-End QA results](#end-to-end-qa-results) — reader + judge, the 85.1% headline
4. [Cross-system published scores](#cross-system-published-scores) — reference tables, non-comparability caveats
5. [Multi-hop vocabulary gap](#multi-hop-vocabulary-gap) — the ~88% retrieval ceiling and what closes it
6. [PERMA personalization benchmark](#perma-personalization-benchmark-proposed) *(proposed)*
7. [AMB LoCoMo cross-evaluation](#amb-locomo-cross-evaluation-idea) *(idea)*
8. [What's measurable but unmeasured](#whats-measurable-but-unmeasured)
9. [Proposed benchmarking experiments](#proposed-benchmarking-experiments)
...
## Multi-hop vocabulary gap

Analysis of the reranker's retrieval ceiling on LoCoMo — the query-versus-evidence vocabulary gap that motivated the Level 5b synthetic vocabulary bridges above. Level 4 17-feature reranker, 2026-03-23.
