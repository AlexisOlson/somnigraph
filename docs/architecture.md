...
#### Shadow-mode write instrumentation (2026-07-01)

The first step toward a write-path quality gate is measurement, not gating (the survey's "measure before you threshold" theme). `remember()` logs one `write_shadow` event to `memory_events` per write attempt, recording the write's nearest same-category neighbors (top-3 memory ids + cosine distance/similarity, with the nearest similarity also in the `similarity_score` column) and the outcome — `inserted`, `superseded`, or `dedup_rejected`. It **reuses the existing dedup KNN pass** (no second vector search) and **changes nothing**: not retrieval, not what gets stored. Rejected writes are logged too, so the ≥0.9-similarity region — the top of the exact distribution a future Write Guard needs — is captured rather than thrown away.

The event uses a distinct `event_type` and a `_write` sentinel `memory_id` (mirroring the `_recall` sentinel used by recall-meta events). Every existing `memory_events` consumer — the reranker's feedback/session/burstiness reads, the Hebbian PMI window, sleep, stats, and the tuning/GT scripts — filters by `event_type`, so `write_shadow` is invisible to all of them; the only unfiltered reads are `stats.py`'s display-only counts, which simply surface a new `write_shadow: N` row. First-of-category writes have no same-category neighbor and are logged with an empty neighbor list (null `similarity_score`) — structurally un-dedupable, and counted separately by the reader.

`scripts/write_shadow_histogram.py` buckets the accumulated similarities by outcome. After weeks of live writes it will produce the production near-duplicate distribution that sets the Write Guard's ADD/UPDATE/SUPERSEDE thresholds empirically instead of by guess — the whole point of shadowing before gating.
