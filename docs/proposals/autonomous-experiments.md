...
4. `eval_retrieval.py --dataset production --configs reranker` before and after; report the delta. (First export `reranker_model.txt`/`reranker_features.json` on the copy — see §Feasibility "Reranker load path"; otherwise both arms measure the formula, not V5+3b.)
...
- **I10. Reranker-loading diagnostic (honest-accounting flag, surfaced by this scoping).** The `.txt`/`.json` artifacts `reranker.py` needs to load V5+3b are **absent from the live `~/.claude/data/tuning_studies/`** (only `.pkl` present) — so the live MCP server is very likely on formula fallback, not the learned reranker STEWARDSHIP describes as "live in production." Confirming this against the running server, and (if confirmed) exporting the `.txt` or fixing the training pipeline to always export it, **touches the live store and the production scoring path** — interactive, and arguably a Priority-1 honest-accounting correction to make before any doc claims "reranker live." (An autonomous experiment can still measure the *formula* fallback cleanly; it just must label it as such.)
...
- **Reranker load path — a real discrepancy, verified on disk.** `reranker.py:_load_model()` loads a **native LightGBM `.txt`** via `lgb.Booster(model_file=MODEL_PATH)`, `MODEL_PATH = DATA_DIR/tuning_studies/reranker_model.txt`, features from `reranker_features.json`. But the live `~/.claude/data/tuning_studies/` currently holds **only `reranker_model.pkl` + `reranker_features.pkl` (3/20) — no `.txt`, no `.json` anywhere under `~/.claude/data`.** Since `MODEL_PATH.exists()` is False, the loader logs "No reranker model — using formula scoring" and **falls back to the formula.** `train_reranker.py:78` *does* export the `.txt` (`final_model.booster_.save_model(MCP_MODEL_PATH)` + features `.json`), so the artifact is regenerable — but as it stands the learned reranker is not loading. **Consequence for experiments:** any run using `--configs reranker` on a copy of `~/.claude/data` measures the *formula*, not V5+3b, unless the `.txt`/`.json` are exported first (a ~5-line pkl→booster→`save_model` converter, or a retrain). Affected artifacts (#1 sleep fork, #4 prospective indexing) bake in an export-or-flag step.
...
6. `reranker_model.txt`/`reranker_features.json` export from the existing `.pkl` (~5 lines: load pkl → `.booster_.save_model(...)` → dump feature names) — needed for any `--configs reranker` run to measure V5+3b instead of the formula. See §Interactive #I10 for the live-store version of this.
