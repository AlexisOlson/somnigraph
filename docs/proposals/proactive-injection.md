...
**Status: proposal, not built.** This document describes a design and an offline experiment to validate its core assumption before any code is written. Read alongside `roadmap.md` (where this is listed as an open question) and `architecture.md` (the pull-based system it extends).
