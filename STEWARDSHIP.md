...
Current state: separate 17-feature LoCoMo-specific reranker (R@10-optimized two-pass selection, distinct from the 26-feature production model in P2), GPT-4.1-mini reader. Level 4: baseline OVERALL R@10=86.3%, MRR=0.693; expanded R@10=88.7%, MRR=0.713. BM25-damped IDF keyword expansion. Two-phase expansion working. Corrected GT vendored from locomo-audit (6.4% ceiling). 3 of 6 expansion methods dead (rocchio 0%, multi_query 2%, entity_focus 4%) — ablation pending.

Key findings: R@10 and NDCG@10 select structurally different feature sets — three features backward-eliminated for NDCG were selected by R@10 forward. Multi-hop failure analysis (22 R@10 misses): 4 near-misses recoverable by reranker, 18 complete whiffs caused by vocabulary gap (88% zero content-word overlap with evidence). The remaining multi-hop failures are retrieval failures, not ranking failures — reranker improvements alone cannot close the gap. See `docs/multihop-failure-analysis.md`.
