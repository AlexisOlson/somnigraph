...
The research program the evidence converged on. Phase 18 found that write-path quality — not retrieval — is what the LoCoMo/LME leaders win on (independent corroboration of the Phase 15 AMemGym finding), and the 59-system survey's three biggest recurring themes are all write-path moves: (1) move reconciliation to the write path, (2) write-path quality / salience / noise gating, (3) secret/PII redaction at ingest. See [`docs/ideas-considered.md`](docs/ideas-considered.md#recurring-themes-the-real-signal) § Recurring themes and [`docs/similar-systems.md`](docs/similar-systems.md#write-path-is-the-lever-2026-06-28-source-scan) § "Write-path is the lever." Somnigraph defers typed-edge and merge work to sleep and has no write-path *quality/salience* gate — `remember()` will embed a low-value or sub-threshold near-duplicate memory today (the existing synchronous dedup check only supersedes/rejects at cosine similarity ≥ 0.9; anything below still lands). Secret/PII redaction, by contrast, **already runs on the write path before embedding** (`privacy.py`) — a 2026-07-01 audit corrected an earlier version of this sentence that claimed `remember()` "will embed a leaked API key today." It will not, for the covered patterns; the redaction call precedes the `embed_text()` call in `impl_remember`, so a redacted secret never enters the vector or FTS index.
...
1. **Shadow-mode near-dup logging + secret redaction — SHIPPED 2026-07-01** (branch `write-path/instruments`). `remember()` now logs one measurement-only `write_shadow` event per write attempt (nearest same-category neighbors + outcome, reusing the existing dedup KNN pass — no second vector search, no behavior change), and `scripts/write_shadow_histogram.py` reads the accumulating distribution. Redaction was audited (it already ran *before* embedding) and its coverage extended to bearer tokens, PEM private-key blocks, credit cards, AWS/Google keys, and more connection-string schemes. Verified on a scratch DB across all outcomes. The near-dup histogram is the measurement that will set step 2's Write Guard thresholds from the observed production distribution rather than a guessed cutoff. See [`docs/sessions/2026-07-01-write-path-instruments.md`](docs/sessions/2026-07-01-write-path-instruments.md) and [`docs/architecture.md`](docs/architecture.md#shadow-mode-write-instrumentation-2026-07-01) § Shadow-mode write instrumentation. **Still needs weeks of live writes to accumulate a real distribution before step 2.**
...
**Status correction (2026-07-01):** the reranker is **NOT currently live** — production has been on the hand-tuned **formula fallback since 2026-04-07**, when a loader format change (pickle → native `.txt`) shipped without the matching artifacts ever landing. This went unnoticed for ~3 months because graceful fallback was silent. The `fix/reranker-restore` branch (2026-07-01) shipped the index hygiene, active candidate-pool filter, NaN-encoding, and an anti-recurrence mechanism (loud fallback warning + `memory_stats()` scorer identity), but **staged no model** — the exact V5+3b recipe is unreproducible (GT + sidecars + probe events gone; see [architecture.md § the silent fallback](docs/architecture.md#the-silent-fallback-apriljuly-2026)). Everything below describes the V5+3b model as trained and evaluated offline — the numbers stand, but that model never served a live query after 2026-04-07.

**Restore update (2026-07-02): the dedicated retrain session ran — V6 is trained, gated, merged (`7ce75e7`), and deployed.** Fresh 4,316-query GT from the current event log (1,423 inactive-memory feedback rows filtered; real `recall_meta` context recovered for 55.9% of queries, vec input ≠ query for 82.7%). All sanity gates passed: **NDCG@5k 0.8997 vs formula 0.8155 on the same GT (+8.42pp)**; held-out probe-hard NDCG 0.9048 / R@10 0.9190 with 0/270 misses; no dead features (`fts_bm25_norm` final-model gain 2424.9). **V6 is not comparable to V5+3b** (different GT) — its baseline is the formula on its own GT. Full recipe + artifacts archived durably in `~/.claude/data/tuning_studies/v6_bundle/` (the V5+3b anti-loss mechanism). **Deployed and confirmed live 2026-07-02**: after the file copy + server restart, `memory_stats()` reports `reranker: 31 features, loaded from ...tuning_studies/reranker_model.txt` and a smoke recall scored through the reranker with no fallback warning — ending the outage that began 2026-04-07. The restore thread is closed; P3 is plain maintenance. Deploy record in [`docs/sessions/2026-07-02-v6-retrain.md`](docs/sessions/2026-07-02-v6-retrain.md).

Historically: LightGBM pointwise reranker ran in production from repo code via directory junction, with the hand-tuned scoring formula preserved as fallback.
...
Current state: Level 5b graph-augmented 15-feature reranker with synthetic coverage scoring (Config K, distinct from the 31-feature production model in P3), GPT-4.1-mini reader. **Level 5b expanded: OVERALL R@10=95.4%, MRR=0.882, R@20=96.9%. Multi-hop R@10=88.8%.** All metrics improved substantially over Level 4: MRR +0.169, R@1 +22.4pp, R@10 +6.7pp, multi-hop R@10 +13.5pp. BM25-damped IDF keyword expansion. Corrected GT vendored from locomo-audit (6.4% ceiling). **Expansion-method ablation COMPLETE (2026-07-02):** the "3 of 6 dead" fire rates (rocchio 0%, multi_query 2%, entity_focus 4%) are a stale ≤200-search-limit instrument; at the 4000 limit the pool holds the whole benchmark DB, so all six methods add zero net-new candidates and the `--expand-all` lift is entirely the Phase-2 rerank pass (proven by arm g: Phase 2 forced with no methods reproduces `--expand-all`, diverges from no-expansion on 827 records). Removability is **not settleable on this benchmark** (it can't exercise candidate expansion); methods stay in code — this expansion machinery is bench-harness-only, not the production recall path. See [`experiments/expansion-ablation/`](experiments/expansion-ablation/findings-expansion-ablation.md) and `docs/benchmarks.md` § Expansion-method ablation.
...
Remaining: sleep pass ablation, feedback loop ablation, document findings in experiments.md. (Expansion-method ablation done — see above.)
...
- 2026-07-02: Sleep counterfactual fork (arc step 4) — the central open question measured. Four arms, 1,032 GT queries, paired bootstrap CIs, formula-scored: **steady-state consolidation is statistically inert on retrieval**; the full cycle's +0.0022 NDCG (0.34% relative) decomposes into two individually-null halves (consolidation / probe feedback). A full `--deep` pass did zero merges/dormancy/archival — the store is saturated. Accumulation-regime re-run registered: two-week abstinence window (started 2026-07-01), fork ~07-15, scored by deployed V6. See [`experiments/sleep-fork/findings-sleep-fork.md`](experiments/sleep-fork/findings-sleep-fork.md) and [`docs/sessions/2026-07-02-sleep-fork.md`](docs/sessions/2026-07-02-sleep-fork.md).
- 2026-07-02: Expansion-method ablation COMPLETE (P5). 6-arm LoCoMo sweep; `--expand-all` reproduced L5b exactly. Corrected finding: the "3 of 6 dead" fire rates are a stale ≤200-search-limit instrument — at the 4000 limit the pool holds the whole DB, all six methods add 0 net-new candidates, and the +4.9pp lift is entirely the Phase-2 rerank (proven by arm g: Phase 2 forced with no methods reproduces `--expand-all`; diverges from no-expansion on 827 records). Removability not settleable on this benchmark; methods stay in code (bench-harness-only). Review pass corrected a "mathematically identical" overclaim to within-noise-floor (3 identical runs differ by 5–9 records). See [`docs/sessions/2026-07-02-expansion-ablation.md`](docs/sessions/2026-07-02-expansion-ablation.md).
- 2026-07-02: V6 reranker retrain (arc step 2.5 part 2) — model built, gated, staged on `fix/reranker-retrain`. Fresh 4,316-query GT from live events (probe-hard holdout survived in the log: 270q). All gates pass: **+8.42pp NDCG@5k vs formula on the same GT** (0.8997 vs 0.8155); probe-hard holdout 0.9048 with 0/270 misses. Not comparable to V5+3b (different GT). March continuity: +1.3pp full / +1.1pp disjoint over today's formula on a drifted instrument. Training inputs archived durably (`v6_bundle/`). Two infra fixes: embed-batch 2048 cap, CV OOM. See [`docs/sessions/2026-07-02-v6-retrain.md`](docs/sessions/2026-07-02-v6-retrain.md).
- 2026-07-01: Proactive-injection offline floor study (arc step 3). Split verdict: the RRF-family score carries real binary surface/skip discrimination (AUC 0.64–0.74 — the cliff's R²<0 null does not carry over), but no floor beats always-inject on F-beta at any β; the log's 0.78–0.88 base rate is a selection artifact (only recall-turns are logged, the proactive population is unobservable offline). Hook not built; a log-only online shadow hook is the honest next instrument if pursued. See [`experiments/floor-study/findings-floor-study.md`](experiments/floor-study/findings-floor-study.md).
- 2026-07-01: Reranker restore (arc step 2.5) — index hygiene + anti-recurrence SHIPPED; retrain HARD-STOPPED. Found the reranker has been on **formula fallback since 2026-04-07** (loader switched to `.txt`, artifacts never landed; silent for ~3 months). Fixed on `fix/reranker-restore` (merged to main, `5cc4f66`): supersede vec/FTS cleanup + self-healing backfill, active candidate-pool filter, NaN-encoded missing-meta block (live+train parity), and a loud fallback warning + `memory_stats()` scorer identity. **Staged no model**: V5+3b is unreproducible (GT + sidecars + probe events gone — it survives only as numbers in experiments.md). Restore split to a dedicated follow-up (fresh GT from current feedback via the V5 pipeline w/ inactive-memory filtering → retrain → sanity-gate vs formula → stage artifacts). Env finding: this machine is OpenAI 1536d, not fastembed. See [`docs/sessions/2026-07-01-reranker-restore.md`](docs/sessions/2026-07-01-reranker-restore.md).
- 2026-07-01: Write-path measurement instruments (P2 step 1, SHIPPED). `remember()` now logs one measurement-only `write_shadow` event per write attempt (nearest same-category neighbors + inserted/superseded/dedup_rejected outcome), reusing the existing dedup KNN pass — no second search, no behavior change; verified isolated from every `event_type`-filtering consumer. Redaction audited (already ran pre-embedding — corrected P2's false "leaked API key today" claim) and extended (PEM keys, bearer/AWS/Google tokens, credit cards). Task 3 (NaN-encode masquerading defaults) STOPPED: the block *can* fire via pending + superseded reranker candidates, so code left untouched and the architecture.md "dead-code" note corrected. Scratch-DB verified. See [`docs/sessions/2026-07-01-write-path-instruments.md`](docs/sessions/2026-07-01-write-path-instruments.md).
