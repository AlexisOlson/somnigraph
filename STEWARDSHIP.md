...
Current state: 15-feature LoCoMo reranker (forward stepwise, NDCG@10-optimized), GPT-4.1-mini reader. Two-phase expansion working — train/eval mismatches fixed (embeddings, temporal regex, theme_overlap), entity extraction stopwords cleaned. Expansion improves R@10 +1.4pp, MRR +0.049 on convs 0-4. 32 features total (3 new: expansion_method_count, phase1_rrf_score, is_seed). Overnight retrain running with forward/backward selection comparison across all 10 conversations. Corrected GT vendored from locomo-audit (6.4% ceiling). 3 of 6 expansion methods are nearly dead (rocchio 0%, multi_query 2%, entity_focus 4%) — ablation pending.
...
Remaining: analyze overnight retrain results, expansion method ablation, rerun with corrected GT, sleep pass ablation, feedback loop ablation, document findings in experiments.md.
